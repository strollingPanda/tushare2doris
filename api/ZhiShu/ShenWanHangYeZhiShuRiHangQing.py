...
def create_table():
    ...
    operation = (
        ...
        + "."
        + config["Ts_ZhiShu_ShenWanHangYeZhiShuRiHangQing"]["table_name"]
        + """
        (
            ...
            float_mv DECIMAL COMMENT "流通市值（万元）",
            total_mv DECIMAL COMMENT "总市值（万元）",
            weight DECIMAL COMMENT "权重"          
        )
        UNIQUE KEY(ts_code,trade_date)
        COMMENT "tushare_指数_申万行业指数日行情"
...
def download():
    ...
    # 下载数据存储的表格名称
    table_name = config["Ts_ZhiShu_ShenWanHangYeZhiShuRiHangQing"]["table_name"]
