...
def create_table():
    ...
    # 下载数据存储的表格名称
    table_name = config["Ts_ZhaiQuan_KeZhuanZhaiPiaoMianLiLv"]["table_name"]
    # 删除原有表格
    ...
    operation = (
        ...
        + "."
        + config["Ts_ZhaiQuan_KeZhuanZhaiPiaoMianLiLv"]["table_name"]
        + """
    (
        ts_code VARCHAR(50) COMMENT "转债代码",
        rate_freq BIGINT COMMENT "付息频率(次/年)",
        rate_start_date VARCHAR(8) COMMENT "纯债价值",
        rate_end_date VARCHAR(8) COMMENT "纯债溢价率（%）",
        coupon_rate DECIMAL COMMENT "票面利率(%)"
    )
    DUPLICATE KEY(ts_code,rate_freq)
    COMMENT "tushare-债券_可转债票面利率"
