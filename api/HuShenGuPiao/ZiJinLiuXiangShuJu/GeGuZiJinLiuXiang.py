...
def create_table():
    ...
    operation = (
        ...
        + "."
        + config["Ts_HuShenGuPiao_ZiJinLiuXiangShuJu_GeGuZiJinLiuXiang"]["table_name"]
        + """
        (
        ...
        COMMENT "tushare_沪深股票_资金流向数据_个股资金流向"
...
def download():
    """下载 沪深股票_资金流向数据_个股资金流向"""

    # 读取config/database.yaml
    config = basis.basis_function.load_config()

    # 下载数据存储的表格名称
    table_name = config["Ts_HuShenGuPiao_ZiJinLiuXiangShuJu_GeGuZiJinLiuXiang"]["table_name"]
