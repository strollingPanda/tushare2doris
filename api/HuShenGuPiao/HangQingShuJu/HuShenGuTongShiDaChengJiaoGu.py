...
def create_table():
    ...
    operation = (
        ...
        + "."
        + config["Ts_HuShenGuPiao_HangQingShuJu_HuShenGuTongShiDaChengJiaoGu"]["table_name"]
        + """
        (
            ...
            change DECIMAL COMMENT "涨跌幅",
            rank VARCHAR(30) COMMENT "资金排名",
            market_type VARCHAR(30) COMMENT "市场类型（1：沪市 3：深市）",
            amount DECIMAL COMMENT "成交金额",
            net_amount DECIMAL COMMENT "净成交金额",
            buy DECIMAL COMMENT "买入金额",
            sell DECIMAL COMMENT "卖出金额",
        )
        UNIQUE KEY(trade_date,ts_code)
        COMMENT "tushare_沪深股票_行情数据_沪深股通十大成交股"
...
def download():
    """下载 沪深股票_行情数据_沪深股通十大成交股"""

    # 读取config/database.yaml
    config = basis.basis_function.load_config()

    # 下载数据存储的表格名称
    table_name = config["Ts_HuShenGuPiao_HangQingShuJu_HuShenGuTongShiDaChengJiaoGu"]["table_name"]
